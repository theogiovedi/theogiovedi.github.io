...
export function rgbToHex(rgb) {
    return "#" + rgb[0].toString(16).padStart(2, "0").toUpperCase() + rgb[1].toString(16).padStart(2, "0").toUpperCase() + rgb[2].toString(16).padStart(2, "0").toUpperCase();
}
...
export function normalToProt(rgb) {
    let red = (((rgb[0] / 255) * 0.152286) + ((rgb[1] / 255) * 1.052583) + ((rgb[2] / 255) * -0.204868)) * 255;
    let green = (((rgb[0] / 255) * 0.114503) + ((rgb[1] / 255) * 0.786281) + ((rgb[2] / 255) * 0.099216)) * 255;
    let blue = (((rgb[0] / 255) * -0.003882) + ((rgb[1] / 255) * -0.048116) + ((rgb[2] / 255) * 1.051998)) * 255;
...
export function normalToDeut(rgb) {
    let red = (((rgb[0] / 255) * 0.367322) + ((rgb[1] / 255) * 0.860646) + ((rgb[2] / 255) * -0.227968)) * 255;
    let green = (((rgb[0] / 255) * 0.280085) + ((rgb[1] / 255) * 0.672501) + ((rgb[2] / 255) * 0.047413)) * 255;
    let blue = (((rgb[0] / 255) * -0.011820) + ((rgb[1] / 255) * 0.042940) + ((rgb[2] / 255) * 0.968881)) * 255;
...
export function normalToTrit(rgb) {
    let red = (((rgb[0] / 255) * 1.255528) + ((rgb[1] / 255) * -0.076749) + ((rgb[2] / 255) * -0.178779)) * 255;
    let green = (((rgb[0] / 255) * -0.078411) + ((rgb[1] / 255) * 0.930809) + ((rgb[2] / 255) * 0.147602)) * 255;
    let blue = (((rgb[0] / 255) * 0.004733) + ((rgb[1] / 255) * 0.691367) + ((rgb[2] / 255) * 0.303900)) * 255;
...
export function kMeans(pixels) {
    ...
    for (let iterations = 0; iterations < maxIterations && !convergence; iterations++) {
        ...
        for (let i = 0; i < pixels.data.length; i += 4) {
            ...
            for (let j = 0; j < k; j++) {
                ...
                euclideanDistance = Math.pow(rgb[0] - centroids[j][0], 2) + Math.pow(rgb[1] - centroids[j][1], 2) + Math.pow(rgb[2] - centroids[j][2], 2);
